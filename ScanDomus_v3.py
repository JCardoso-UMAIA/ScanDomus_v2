    ...
    try:
        ...
        if tshark_interfaces:
            ...
            for interface_name in tshark_interfaces:
                ...
                # FIX #5: comparação consistente em lowercase
            ...
            try:
                ...
                for line in result.stdout.strip().split('\n'):
                    if line.startswith('Name='):
                        ...
                        # FIX #5: usar .lower() para comparação consistente
...
def _decode_dns_name(packet, offset, jumps=0):
    ...
    while position < len(packet):
        ...
        # FIX #4: ponteiro de compressão DNS — retorna posição pós-ponteiro (offset + 2)
        if (length & 0xC0) == 0xC0:
            ...
            position += 2  # avança além do ponteiro e termina
            break
...
def _extract_mdns_name_from_packet(packet):
    ...
    try:
        ...
        for _ in range(qdcount):
            ...
            # FIX #7: verificar bounds antes de avançar
...
def discover_ssdp_names(timeout_seconds=1.2):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP) as sock:
            sock.settimeout(timeout_seconds)
            # FIX #6: bind antes de enviar para garantir receção das respostas SSDP
            try:
                sock.bind(('', 0))
            except Exception:
                sock.bind(('0.0.0.0', 0))
...
def _capture_with_tshark_cli(interface_candidate, timeout):
    ...
    for line in (result.stdout or '').splitlines():
        ...
        # FIX #1: usar o timestamp real do pacote em vez de datetime.now()
...
def analyze_traffic(packets, devices=None):
    ...
    # FIX #3: substituir loop O(n²) por compreensão vetorizada
    suspicious = {
...
# FIX #2: inicializar todos os estados de sessão, incluindo 'devices'
...
# FIX #10: renomeado para refletir o comportamento real (atualização manual)

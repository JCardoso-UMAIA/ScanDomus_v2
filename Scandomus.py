...
# Configurar event loop para Windows (suporta subprocess) - Python 3.14+
# Removido para Python 3.14 (deprecated) - asyncio.run() manipula automaticamente
    ...
    try:
        ...
        if sys.platform == 'win32':
            ...
            try:
                ...
                for line in result.stdout.strip().split('\n'):
                    if line.startswith('Name='):
                        ...
                        if name and 'Loopback' not in name and 'etwdump' not in name:
            ...
            try:
                ...
                interfaces = []
                for line in result.stdout.strip().split('\n'):
                    # Linhas com interfaces conectadas têm "Connected" no início
                    if 'Connected' in line or 'Dedicated' in line:
                        # Extrair nome da interface (último campo)
                        parts = line.split()
                        if parts:
...
def _decode_dns_name(packet, offset, jumps=0):
    ...
    return '.'.join([label for label in labels if label]), position if not consumed else position
...
def _create_capture_event_loop():
    if sys.platform == 'win32':
        try:
...
def analyze_traffic(packets, devices=None):
    ...
    for ip in traffic_by_ip['src']:
        if traffic_by_ip[traffic_by_ip['src'] == ip]['packets'].values[0] > 100:  # threshold arbitrário
            suspicious[ip] = "Tráfego elevado detetado"
...
if st.button("Capturar Tráfego"):
    with st.spinner(f"A capturar tráfego durante {traffic_timeout} segundos..."):
        ...
        traffic_by_ip, suspicious = analyze_traffic(packets, st.session_state.get('devices'))
...
# --- Monitorização Contínua (opcional) ---
if st.checkbox("Monitorização Contínua (atualiza a cada 10 segundos)"):
    # Para evitar o problema de event loop em threads, vamos usar uma abordagem diferente
    st.info("Monitorização contínua ativada. Use o botão abaixo para atualizar manualmente.")
    # A monitorização contínua será feita com um botão separado para evitar problemas de event loop
